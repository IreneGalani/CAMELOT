	...
	public QuestRejected(QuestChoiceData qd, Current_Game curr, Special_Rules a, VotingData vd, QuestOutcome qo, Score scr) {
		...
		NextButton.addActionListener(new ActionListener() {
			public void actionPerformed(ActionEvent e) {
				...
				if(quest ==1) {
					newQuestData.setCountQuest(1);
					if(a.checkTargeting()) {
						dispose();
						ChooseQuest cq = new ChooseQuest(newQuestData, curr, a, vd, qo, scr);
						cq.setVisible(true);
					} else {
						dispose();
						ChooseMissioners cm = new ChooseMissioners(newQuestData, curr, a, vd, qo, scr);
						cm.setVisible(true);
					}
				} else if(quest ==2) {
